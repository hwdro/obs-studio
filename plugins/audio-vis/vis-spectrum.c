	...
	uint32_t channels)
{
	...
	context->fft = audio_fft_create(sample_rate, channels, size, oct_fract,
		wg_type);
...
static void spectrum_process_audio(void *data, audio_buffer_t *audio)
{
	...
	audio_fft_new_data(context->fft, audio);
}
	...
	uint32_t color)
{
	...
	if (!sp_rect_intersection(bar, area, &res))
		return;
...
static void spectrum_draw(spectrum_visual_t *context)
{
	...
	audio_fft_build_spectrum(context->fft, context->frame_time);

	sp_memset32(framebuffer, context->bg_color, cx * cy);
	
	uint32_t bands = context->fft->bins - 1;

	sp_rect_t view = { 0, 0, cx, cy };
	...
	for (uint32_t b = 0; b < bands; b++) {
		bar.x  = bars[b].b_x;
		bar.cy = (int)(bars[b].b_y * context->fft->spectrum[b]);
		bar.y  = bars[b].b_y - bar.cy;
		bar.cx = bars[b].width;
