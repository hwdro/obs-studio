		...
		size_t sample_rate)
{
	...
	for (size_t i = 0; i < source->audio_actions.num; i++) {
		...
		if (new_frame_num < frame_num) {
			for (size_t i = frame_num; i < new_frame_num; i++)
				vol_data[i] = cur_vol;
	...
	for (size_t i = frame_num; i < AUDIO_OUTPUT_FRAMES; i++)
		vol_data[i] = cur_vol;
...
static void apply_audio_volume(obs_source_t *source, size_t channels,
		size_t sample_rate)
{
	struct audio_action action;
...
static void custom_audio_render(obs_source_t *source, size_t channels,
		size_t sample_rate)
{
	...
	success = source->info.audio_render(source->context.data, &ts,
			&audio_data, channels, sample_rate);
	source->audio_ts = success ? ts : 0;
	source->audio_pending = !success;

	if (!success || !source->audio_ts)
		return;
	...
	apply_audio_volume(source, channels, sample_rate);
}
		...
		size_t channels, size_t sample_rate, size_t size)
{
	...
	if (source->info.audio_render) {
		custom_audio_render(source, channels, sample_rate);
		return;
	}
	...
	process_audio_source_tick(source, channels, sample_rate, size);
}
