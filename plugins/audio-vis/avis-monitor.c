...
static void avis_monitor_data_received_signal(void *vptr,
	calldata_t *calldata);
static void avis_monitor_volume_signal(void *vptr, calldata_t *calldata);
...
void avis_monitor_acquire_obs_source(avis_monitor_t *monitor)
{
	...
	monitor->volume = obs_source_get_volume(src);
	
	signal_handler_connect(sh, "volume",
		avis_monitor_volume_signal, monitor);
...
static void avis_monitor_volume_signal(void *vptr, calldata_t *calldata)
{
	...
	monitor->volume = (float)calldata_float(calldata, "volume");
}
	...
	calldata_t *calldata)
{
	...
	frames = data->frames;
	m_data->volume = calldata_bool(calldata, "muted") ?
		0.0f : monitor->volume;
